...
st.warning("⚠️ API hosted on Render free tier — first request may take 30–60 seconds to wake up.")
...
with tab1:
    ...
    with col1:
        ...
        home_form = st.slider("Home Form — avg goals scored in last 5 games (0=poor, 5=excellent)", 0.0, 5.0, 1.5)
        home_density = st.slider("Home Fixture Density — games played in last 14 days (higher = more fatigued)", 0, 6, 2)
        b365h = st.number_input("Bet365 Home Win Odds — lower = more likely to win (e.g. 1.5 = heavy favourite)", value=2.0, min_value=1.0)
        b365d = st.number_input("Bet365 Draw Odds — typical range 3.0–4.0", value=3.4, min_value=1.0)

    with col2:
        st.subheader("✈️ Away Team")
        away_form = st.slider("Away Form — avg goals scored in last 5 games (0=poor, 5=excellent)", 0.0, 5.0, 1.2)
        away_density = st.slider("Away Fixture Density — games played in last 14 days (higher = more fatigued)", 0, 6, 2)
        b365a = st.number_input("Bet365 Away Win Odds — lower = more likely to win (e.g. 4.0 = underdog)", value=3.8, min_value=1.0)
        elo_delta = st.number_input("ELO Delta — home team SPI minus away team SPI (positive = home stronger)", value=0.0)
    ...
    if st.button("Predict Match Outcome"):
        with st.spinner("Calling API..."):
            ...
            r = requests.post(f"{API}/predict-match", json=payload, timeout=60)
        ...
        if r.status_code == 200:
            ...
            with col_a:
                ...
                ax.bar(["Away", "Draw", "Home"], probs.values(), color=["#e74c3c", "#3498db", "#2ecc71"])
            ...
            st.error(f"API error {r.status_code} — Render may be sleeping. Wait 30 seconds and try again.")
...
with tab2:
    ...
    **How to find a Player ID:**
    ...
    player_id = st.number_input("Player ID (integer from European Soccer DB)", value=30893, step=1, help="Try 30893 for Messi")

    if st.button("Rate Player"):
        with st.spinner("Calling API..."):
            r = requests.get(f"{API}/rate-player/{int(player_id)}", timeout=60)

        if r.status_code == 200:
            data = r.json()
            st.metric(label="Overall Performance Score", value=f"{data['performance_score']:.1f} / 100")
            stats = data["stats"]
            ...
            st.error("API sleeping or player not found. Wait 30 seconds and retry.")
...
with tab3:
    ...
    **6 Discovered Archetypes:**
    ...
    if st.button("Load Archetype Scatter Plot"):
        with st.spinner("Calling API..."):
            r = requests.get(f"{API}/cluster-archetypes", timeout=60)

        if r.status_code == 200:
            df = pd.DataFrame(r.json())
            colors = {
                "Defensive Anchor": "#e74c3c", "Box-to-Box Midfielder": "#3498db",
                "False 9": "#2ecc71", "Target Striker": "#f39c12",
                "Creative Playmaker": "#9b59b6", "Pressing Forward": "#1abc9c"
            }
            fig, ax = plt.subplots(figsize=(5, 4))
            for archetype, group in df.groupby("archetype"):
                ax.scatter(group["pca_x"], group["pca_y"], label=archetype, alpha=0.6, color=colors.get(archetype, "#999"))
            ...
            st.error("API sleeping — wait 30 seconds and retry.")
...
with tab4:
    ...
    metric = st.selectbox("Select stat to rank by",
        ["finishing", "dribbling", "short_passing", "sprint_speed", "stamina", "vision", "marking"],
    ...
    if st.button("Get Top Performers"):
        with st.spinner("Calling API..."):
            r = requests.get(f"{API}/top-performers?metric={metric}&limit={limit}", timeout=60)
